...
"  var rows=[];"
"  var stColors={tracking:'#00e676',alert:'#f44336',possible:'#ffc107'};"
"  var stLabels={tracking:'TRACKING',alert:'ALERT',possible:'POSSIBLE'};"
"  for(var i=0;i<tracks.length;i++){"
"    var t=tracks[i];"
"    var sc=stColors[t.st]||'#fff';"
"    var alpha=Math.min(1,0.5+t.s*0.5);"
"    ctx.lineWidth=t.st==='alert'?5:3;"
"    ctx.strokeStyle=sc;"
"    ctx.strokeRect(t.x,t.y,t.w,t.h);"
"    var lbl=(stLabels[t.st]||'?')+' #'+t.id;"
"    var lw=ctx.measureText?ctx.measureText(lbl).width+12:120;"
"    ctx.fillStyle='rgba(0,0,0,0.7)';"
"    ctx.fillRect(t.x,t.y-22,lw,22);"
"    ctx.fillStyle=sc;"
"    ctx.font='bold 13px monospace';"
"    ctx.fillText(lbl,t.x+4,t.y-6);"
"    rows.push('<div style=\"color:'+sc+'\">"
"'+lbl+' '+t.w+'x'+t.h+' s='+t.s.toFixed(3)+' age='+t.age+' h='+t.hits+'</div>');"
"  }"
"  if(lastMotion.det){"
"    var hasAlert=tracks.some(function(x){return x.st==='alert';});"
"    var hasPoss=tracks.some(function(x){return x.st==='possible';});"
"    var badge=hasAlert?'ALERT':'MOTION';"
"    var badgeClr=hasAlert?'rgba(244,67,54,0.9)':hasPoss?'rgba(255,193,7,0.85)':'rgba(0,230,118,0.85)';"
"    ctx.fillStyle=badgeClr;"
"    ctx.fillRect(0,0,160,28);"
"    ctx.fillStyle='#fff';"
"    ctx.font='bold 16px monospace';"
"    ctx.fillText(badge+' x'+tracks.length,6,20);"
"  }"
"  /* ── Face overlays ──────────────────────────────────────────── */"
"  if(lastFace&&lastFace.faces){"
"    var faces=lastFace.faces;"
"    for(var fi=0;fi<faces.length;fi++){"
"      var f=faces[fi];"
"      var fc=f.id>0?'#00e676':f.id<0?'#f44336':'#03a9f4';"
"      ctx.lineWidth=3;"
"      ctx.strokeStyle=fc;"
"      ctx.strokeRect(f.x,f.y,f.w,f.h);"
"      var nm=f.name||'?';"
"      var lbl2=nm+(f.sim>0?' '+Math.round(f.sim*100)+'%':'');"
"      var tw2=ctx.measureText?ctx.measureText(lbl2).width+12:80;"
"      ctx.fillStyle='rgba(0,0,0,0.75)';"
"      ctx.fillRect(f.x,f.y+f.h,tw2,20);"
"      ctx.fillStyle=fc;"
"      ctx.font='bold 14px monospace';"
"      ctx.fillText(lbl2,f.x+4,f.y+f.h+15);"
"      rows.push('<div style=\"color:'+fc+'\">FACE '+nm+' sim='+(f.sim||0).toFixed(2)+' score='+(f.score||0).toFixed(2)+'</div>');"
"    }"
"  }"
"  motionList.innerHTML="
"    '<div style=\"color:#69f0ae\">fps '+(lastMotion.fps||0)+"
"    '  score '+((lastMotion.score||0).toFixed(3))+"
"    '  tracks '+tracks.length+"
"    (lastFace?' faces '+(lastFace.faces?lastFace.faces.length:0)+"
"    ' ['+lastFace.mode+']':'')+'</div>'"
"    +rows.join('');"
"}"
...
"        if(msg.t==='motion')lastMotion=msg;"
"        else if(msg.t==='face')lastFace=msg;"
"        drawOverlay();"
"      }catch(e){}"
"      return;"
"    }"
